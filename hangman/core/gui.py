...
def game_play(word,wrd_txt):
    ...
    def done():
        ...
        hangman.destroy()
    def yes():
        string1=tk.Label(text='At what positions?',bg=text_bg,fg=text_col,font=("Arial", 13)).place(x=10,y=110)
...
def game_play_word_check(word):
    ...
    def no():
        hangman.destroy()
        a.append('n')
    string1=tk.Label(text=f'Is the word {word}?',bg=text_bg,fg=text_col,font=("Arial", 13)).place(x=10,y=10)
    button=tk.Button(text="Yes", command=yes,bg=btn_bg,fg=btn_fg,activebackground=btn_active_bg,activeforeground=btn_active_fg).place(x=10,y=40)
    button=tk.Button(text="No", command=no,bg=btn_bg,fg=btn_fg, activebackground=btn_active_bg,activeforeground=btn_active_fg).place(x=140,y=40)
    tk.Button(text="QUIT",command=exit,bg='black',fg='#f2665c',activebackground='#f2665c').place (x=10,y=250)
    hangman.title("Hangman Solver")
    hangman.geometry('750x300')
    hangman.mainloop()
...
def pc_h_intro():
    ...
    def pc_play():
        a.append('2')
        hangman.destroy()
    string1=tk.Label(text="What do you want to do today?", bg=text_bg,fg=text_col,font=("Arial", 13)).place(x=10,y=5)
